...
> **한 줄 요약** — 노션은 기획자에게 단순한 '메모 앱'이 아니다. 기획 문서가 워드, 엑셀, 슬랙, 피그마에 흩어져 있으면 정작 일보다 "그게 어디 있더라"를 찾는 데 시간을 더 쓰게 되는데, 노션은 이 흩어진 것들을 한곳에 모으고 데이터베이스로 서로 엮어준다. 기획자에게 노션이 중요한 건 그래서다.
{: .prompt-tip }
...
노션을 본격적인 기획 도구로 만드는 건 표 기능이다. 노션의 표는 엑셀과 달리 데이터베이스로 동작한다.
...
데이터베이스를 하나 만들어두면 그걸 표로도, 보드로도, 캘린더로도 볼 수 있다. 데이터는 그대로 두고 보기 방식만 바꿔 끼우는 거다.
...
> 개발자한테는 보드를, 디자이너한테는 캘린더를, 팀장한테는 타임라인을 보여줄 수 있다. 문서를 세 벌 따로 만드는 대신 같은 데이터에 뷰만 세 개 걸어두는 거라, 한쪽을 고치면 나머지도 알아서 맞춰진다.
{: .prompt-info }

## 데이터베이스를 연결한다: Relation과 Rollup

노션에는 데이터베이스끼리 연결(Relation)하고, 연결된 값을 끌어다 계산(Rollup)하는 기능이 있다. 단순 문서 도구에는 없는 부분이다.
...
> 이렇게 엮어두면 기획 문서가 지금 상태를 그대로 보여주는 현황판처럼 굴러간다. 한 번 쓰고 덮어두는 일반 문서로는 잘 안 되는 부분이다.
{: .prompt-info }
...
기획은 절반이 커뮤니케이션이다. 노션은 그 논의를 메신저 대신 문서 안에서 하게 해준다.
